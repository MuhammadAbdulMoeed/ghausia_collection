...
//        dd($request->all());
        ...
        if ($request->has('logo')) {
            try {
                ...
                $data['logo'] = ImageUploadHelper::uploadImage($request->logo, 'upload/logo/');
            } catch (\Exception $e) {
                request()->session()->flash('error', 'Error in Saving Logo: ' . $e->getMessage());
            }
        }
        if ($request->has('photo')) {
            try {
                if (isset($settings->photo)) {
                    if (file_exists(public_path($settings->photo))) {
